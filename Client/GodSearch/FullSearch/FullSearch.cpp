		...
		if(update){
			...
			if(tmlCount == maxCount){	//十万回終端を探索しても解が改善されないなら
				...
				if(bdStack.size() > 2){
					...
					freeSize += RemoveShitNums(map, 2, width, height);
					putShitNum = 2;
				} else if(bdStack.size() == 1){
					delete bdStack.top();
					bdStack.pop();
					aStack.pop_back();
					freeSize += RemoveShitNums(map, 1, width, height);
					putShitNum = 1;
				}
				//状態をクリア
				/*
