...
std::vector<wdElement*>* BreadthMake(std::vector<wdElement*>* breadthData, const int* Map, const int width, const int height, const int stonesNum, const int putShitNum, int freeSize, int*** shitAry, const int* shitSizeAry, const int kn, int level){
	static int areaAry[1025];	//下の方でつかいまわしている
	static int subAreaMap[2048];
	int* map = new int[(width+1)*height];
	for(int i = 0; i < (width+1)*height; i++) map[i] = Map[i];
	...
	for(int st = 0; st < 8; st++){
		...
		for(int i = 0; i < baseAry.size(); i++){
			...
			if(JudgePutable(map, shit, basePoint, width, height)){
				...
				printf("level-%d | [%d,%d] :", level, st, basePoint);
				*/

				if(kn < stonesNum-1 && level < LEVEL-1){
					stAry[level] = st;
					bsAry[level] = basePoint;
					BreadthMake(breadthData, map, width, height, stonesNum, putShitNum+1, freeSize, shitAry, shitSizeAry, kn+1, level+1);
				} else {
					int needShitNum = 0;
					int fit;
					int areaNum;
					...
					//配置情報
					temp->level = level+1;
					temp->st = new int[level+1];//st;
					temp->basePoint = new int[level+1];//basePoint;
					for(int j = 0; j < level; j++){
						temp->st[j] = stAry[j];
						temp->basePoint[j] = bsAry[j];
					} 
					temp->st[level] = st;
					temp->basePoint[level] = basePoint;
					...
					printf("dead %d : %d %d", deadArea, fit, neighborNum);
					DEBUG_waitKey();
					*/
				}
...
std::vector<wdElement*>* BreadthMake(std::vector<wdElement*>* breadthData, std::vector<int> startNeighbor, const int* Map, const int width, const int height, const int stonesNum, const int putShitNum, int freeSize, int*** shitAry, const int* shitSizeAry, const int kn, int level){
	static int areaAry[1025];	//下の方でつかいまわしている
	static int subAreaMap[2048];
	int* map = new int[(width+1)*height];
	for(int i = 0; i < (width+1)*height; i++) map[i] = Map[i];
...
RESTART :
	...
	PUT_PROCESS:
			...
			int remove = RemoveShitNums(map, kn, width, height);
			freeSize += remove;
			if(remove != 0) {
				putShitNum--;
				aStack.pop_back();
			...
			for(int l = 0; l < data->level; l++){
				int st = data->st[l];
				int placePoint = data->basePoint[l];
...
//		printf("aa\n");
		...
		if(startFlag){
			breadthData = BreadthMake(breadthData, startNeighbor, map, width, height, stonesNum, putShitNum, freeSize, shitAry, shitSizeAry, kn, 0);
		} else {
			breadthData = BreadthMake(breadthData, map, width, height, stonesNum, putShitNum, freeSize, shitAry, shitSizeAry, kn, 0);
		}
//		printf("ee\n");
...
#ifdef DEBUG_CODE
#ifdef CHECK_CODE
			printf("shit %d(%c) place pattern %d\n", kn, kn+49, breadthData->size()); 
#endif
#endif
//			for(int i = 0; i < breadthData->size() && (breadthData->data())[i]->rank == 0; i++){
			...
			for(int i = 0; i < breadthData->size(); i++){
				...
				printf("\t%d | %d dead [%d %d]\n", elm->rank, elm->deadArea, elm->minAreaSize, elm->neighborNum);
				for(int j = 0; j < elm->level; j++){
					printf("\t\t%d | %d-(%d,%d)\n", j, elm->st[j], elm->basePoint[j]%(width+1), elm->basePoint[j]/(width+1));
				}
				DEBUG_waitKey();
			}
#endif
